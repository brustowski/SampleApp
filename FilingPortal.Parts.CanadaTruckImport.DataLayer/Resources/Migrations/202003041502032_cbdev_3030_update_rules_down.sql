...
-- restore default value for Transport Mode and Service Option
DECLARE @command VARCHAR(MAX);

SELECT
  @command = COALESCE(@command, '') + 'ALTER TABLE canada_imp_truck.declaration DROP CONSTRAINT ' + d.name + ';' + CHAR(10)
FROM sys.columns AS c
JOIN sys.default_constraints AS d
  ON d.parent_object_id = c.object_id
    AND d.parent_column_id = c.column_id
WHERE c.name IN ('ro_service', 'st_transport')
AND c.object_id = OBJECT_ID('canada_imp_truck.declaration', 'U');
