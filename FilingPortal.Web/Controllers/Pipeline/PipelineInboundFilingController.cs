...
{
    /// <summary>
    /// Provides actions for Pipeline Inbound Records 
    /// </summary>
    [RoutePrefix("api/inbound/pipeline/filing")]
    public class PipelineInboundFilingController : ApiControllerBase
    {
        /// <summary>
        /// The filing procedure service
        /// </summary>
        private readonly IPipelineFilingService _procedureService;
        ...
        /// <summary>
        /// Pipeline filing configuration factory
        /// </summary>
        private readonly IFilingConfigurationFactory<PipelineDefValueManualReadModel> _configurationFactory;

        /// <summary>
        /// Initializes a new instance of the <see cref="PipelineInboundFilingController"/> class
        /// </summary>
        /// <param name="procedureService">The filing procedure service</param>
        /// <param name="filingHeaderRepository">The filing header repository</param>
        ...
        public PipelineInboundFilingController(
            IPipelineFilingService procedureService,
            IPipelineFilingHeaderRepository filingHeaderRepository,
            IFilingConfigurationFactory<PipelineDefValueManualReadModel> configurationFactory,
        ...
        public IHttpActionResult Start([FromBody] int[] ids)
        {
            using (new MonitoredScope("Starting Filing process"))
            {
                ...
                List<PipelineFilingHeader> filingHeaders = _filingHeaderRepository.FindByInboundRecordIds(ids).Where(x => x.CanBeEdited).ToList();
                foreach (var id in ids)
                {
                    PipelineFilingHeader currentHeader = filingHeaders.FirstOrDefault(x => x.PipelineInbounds.Any(y => y.Id == id));
        ...
        {
            IEnumerable<PipelineFilingHeader> filingHeaders = _filingHeaderRepository.GetList(filingHeadersId);
        ...
        public IHttpActionResult CancelFilingProcess(IEnumerable<int> ids)
        {
            ...
            {
                IEnumerable<PipelineFilingHeader> filingHeaders;
        ...
        {
            return _filingHeaderRepository.GetList(filingHeaderIds).SelectMany(x => x.PipelineInbounds.Select(p => p.Id));
        }
