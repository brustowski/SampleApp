...
{
    [TestClass]
    public class TruckExportFilingActionsControllerTests : ControllerBaseTests<TruckExportFilingActionsController>
    {
        private Mock<ITruckExportFilingService> _fileProcedureServiceMock;
        private Mock<IListInboundValidator<TruckExportReadModel>> _inboundRecordValidatorMock;
        private Mock<IFilingHeaderDocumentUpdateService<TruckExportDocumentDto>> _documentFileProcedureServiceMock;
        private Mock<IDefValuesManualValidator<TruckExportDefValuesManualReadModel>> _modelValidatorMock;
        private Mock<IFilingParametersService<TruckExportDefValueManual, TruckExportDefValuesManualReadModel>> _paramsServiceMock;

        protected override TruckExportFilingActionsController CreateControllerTestInitialize()
        {
            _fileProcedureServiceMock = new Mock<ITruckExportFilingService>();
            _inboundRecordValidatorMock = new Mock<IListInboundValidator<TruckExportReadModel>>();
            _documentFileProcedureServiceMock = new Mock<IFilingHeaderDocumentUpdateService<TruckExportDocumentDto>>();
            _modelValidatorMock = new Mock<IDefValuesManualValidator<TruckExportDefValuesManualReadModel>>();
            _paramsServiceMock = new Mock<IFilingParametersService<TruckExportDefValueManual, TruckExportDefValuesManualReadModel>>();

            SetupValidator(new[] { 45 }, new int[0]);

            return new TruckExportFilingActionsController(
        ...
        public void File_CallsService_AddDocumentsToFilingHeader()
        {
            ...
            _documentFileProcedureServiceMock.Verify(x 
                => x.UpdateForFilingHeader(45, It.Is((List<TruckExportDocumentDto> c) => c.All(d => d.FileDesc == "Desc")), true), Times.Once);
        }
        ...
        public void Save_CallsService_AddDocumentsToFilingHeader()
        {
            ...
            _documentFileProcedureServiceMock.Verify(x => x.UpdateForFilingHeader(89,
                It.Is((List<TruckExportDocumentDto> c) => c.All(d => d.FileDesc == "doc description")), false), Times.Once);
        }
