...
BEGIN
  ...
  (SELECT
     ...
     ,importer_set.found AS importer_set
     ,(SELECT
          [name] AS message
        FROM (VALUES (
        CASE rule_importer.found
          ...
          ELSE NULL
        END), (
        CASE importer_set.found
          WHEN 0 THEN importer_set.message
          ELSE NULL
        END
        )) X ([name])
        FOR JSON PATH)
    ...
    CROSS APPLY (SELECT
        IIF(inbnd.importer_id IS NULL, 0, 1) AS found
       ,'Importer is not set for EIN = "' + inbnd.ein + '"' AS message) AS importer_set)

  UPDATE inbnd
  SET validation_passed = t.importer_found & t.importer_set
