--
-- Create table [zones_entry].notes
--
CREATE TABLE zones_entry.notes (
  id int IDENTITY,
...
--
-- Create index [Idx__filing_header_id] on table [zones_entry].notes
--
CREATE INDEX Idx__filing_header_id
  ON zones_entry.notes (filing_header_id)
  INCLUDE (id)
  ON [PRIMARY]
GO

--
-- Create index [Idx__parent_record_id] on table [zones_entry].notes
--
CREATE INDEX Idx__parent_record_id
  ON zones_entry.notes (parent_record_id)
  INCLUDE (id)
  ON [PRIMARY]
GO

--
-- Create foreign key [FK__zones_entry__notes__zones_entry__filing_header__filing_header_id] on table [zones_entry].notes
--
ALTER TABLE zones_entry.notes
  ADD CONSTRAINT FK__zones_entry__notes__zones_entry__filing_header__filing_header_id FOREIGN KEY (filing_header_id) REFERENCES zones_entry.filing_header (id)
GO

--
-- Create procedure [zones_entry].[sp_add_notes]
--
GO
-- add notes record --
CREATE PROCEDURE zones_entry.sp_add_notes (@filingHeaderId INT,
@parentId INT,
...
BEGIN
  ...
  IF @allowMultiple = 1
    OR NOT EXISTS (SELECT
        1
      FROM zones_entry.notes notes
      WHERE notes.filing_header_id = @filingHeaderId)
  BEGIN
...
-- add filing records --
ALTER PROCEDURE zones_entry.sp_create_entry_records (@filingHeaderId INT,
@filingUser NVARCHAR(255) = NULL)
AS
BEGIN
  ...
  EXEC zones_entry.sp_add_packing @filingHeaderId
                                 ...
                                 ,@operationId
  -- add notes
  EXEC zones_entry.sp_add_notes @filingHeaderId
                                 ,@filingHeaderId
                                 ,@filingUser
                                 ,@operationId
END;
...
GO
INSERT zones_entry.form_section_configuration(id, name, title, table_name, procedure_name, is_array, display_as_grid, is_hidden, parent_id) VALUES (8, 'notes', 'Notes', 'zones_entry.notes', 'sp_add_notes', CONVERT(bit, 'False'), CONVERT(bit, 'False'), CONVERT(bit, 'False'), 1)
GO
SET IDENTITY_INSERT zones_entry.form_section_configuration OFF
GO

INSERT zones_entry.form_configuration(section_id, depends_on_id, confirmation_needed, column_name, created_date, created_user, display_on_ui, manual, editable, mandatory, single_filing_order, description, label, paired_field_table, paired_field_column, handbook_name, has_default_value, overridden_type) VALUES (8, NULL, CONVERT(bit, 'False'), 'title', GETDATE(), SUSER_NAME(), 1, 0, CONVERT(bit, 'True'), CONVERT(bit, 'False'), NULL, NULL, 'Title', NULL, NULL, NULL, CONVERT(bit, 'False'), NULL)
INSERT zones_entry.form_configuration(section_id, depends_on_id, confirmation_needed, column_name, created_date, created_user, display_on_ui, manual, editable, mandatory, single_filing_order, description, label, paired_field_table, paired_field_column, handbook_name, has_default_value, overridden_type) VALUES (8, NULL, CONVERT(bit, 'False'), 'date', GETDATE(), SUSER_NAME(), 2, 0, CONVERT(bit, 'True'), CONVERT(bit, 'False'), NULL, NULL, 'Date', NULL, NULL, NULL, CONVERT(bit, 'False'), NULL)
INSERT zones_entry.form_configuration(section_id, depends_on_id, confirmation_needed, column_name, created_date, created_user, display_on_ui, manual, editable, mandatory, single_filing_order, description, label, paired_field_table, paired_field_column, handbook_name, has_default_value, overridden_type) VALUES (8, NULL, CONVERT(bit, 'False'), 'message', GETDATE(), SUSER_NAME(), 3, 0, CONVERT(bit, 'True'), CONVERT(bit, 'False'), NULL, NULL, 'Message', NULL, NULL, NULL, CONVERT(bit, 'False'), NULL)
GO
...
SELECT
 ...
 ,notes.date AS notes_date
 ,notes.message AS notes_message
...
LEFT JOIN zones_entry.notes notes
  ON header.id = notes.filing_header_id
