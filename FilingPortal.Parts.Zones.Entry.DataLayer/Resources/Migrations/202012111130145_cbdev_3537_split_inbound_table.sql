...
--
-- Create column [importer_id] on table [zones_entry].[declaration]
--
ALTER TABLE zones_entry.declaration
  ADD importer_id uniqueidentifier NULL
GO

--
-- Create column [filer_code] on table [zones_entry].[declaration]
--
ALTER TABLE zones_entry.declaration
  ADD filer_code varchar(128) NULL DEFAULT 'BER'
GO

--
-- Add default constraint on table [zones_entry].[declaration]
--
ALTER TABLE zones_entry.declaration
  ADD DEFAULT ('06') FOR entry_type
GO
...
--
-- Create column [team_no] on table [zones_entry].[misc]
--
ALTER TABLE zones_entry.misc
  ADD team_no varchar(3) NULL
GO
...
ALTER TABLE zones_entry.invoice_line
  ADD line_no int NULL
GO

UPDATE zones_entry.invoice_line 
SET line_no = invoice_line_number
GO
...
--
-- Create column [ftz_date] on table [zones_entry].[invoice_line]
--
ALTER TABLE zones_entry.invoice_line
  ADD ftz_date datetime NULL
GO
...
DECLARE @misc_id INT = NULL;
SELECT
  @misc_id = fsc.id
FROM zones_entry.form_section_configuration fsc
WHERE fsc.table_name = 'zones_entry.misc'
...
INSERT zones_entry.form_configuration (section_id, column_name, created_user, display_on_ui, manual, editable, mandatory, description, label, handbook_name, confirmation_needed, has_default_value)
  VALUES (@decl_id, 'filer_code', SUSER_NAME(), 252, 0, CONVERT(BIT, 'True'), CONVERT(BIT, 'False'), NULL, 'Filer Code', null, CONVERT(BIT, 'False'), CONVERT(BIT, 'True'))
INSERT zones_entry.form_configuration (section_id, column_name, created_user, display_on_ui, manual, editable, mandatory, description, label, handbook_name, confirmation_needed, has_default_value)
  VALUES (@misc_id, 'team_no', SUSER_NAME(), 252, 0, CONVERT(BIT, 'True'), CONVERT(BIT, 'False'), NULL, 'Team No', null, CONVERT(BIT, 'False'), CONVERT(BIT, 'False'))
INSERT zones_entry.form_configuration (section_id, column_name, created_user, display_on_ui, manual, editable, mandatory, description, label, handbook_name, confirmation_needed, has_default_value)
  VALUES (@lines_id, 'ftz_date', SUSER_NAME(), 252, 0, CONVERT(BIT, 'True'), CONVERT(BIT, 'False'), NULL, 'FTZ Date', null, CONVERT(BIT, 'False'), CONVERT(BIT, 'False'))
INSERT zones_entry.form_configuration (section_id, column_name, created_user, display_on_ui, manual, editable, mandatory, description, label, handbook_name, confirmation_needed, has_default_value)
  VALUES (@lines_id, 'line_no', SUSER_NAME(), 1, 0, CONVERT(BIT, 'False'), CONVERT(BIT, 'False'), NULL, 'Line No', null, CONVERT(BIT, 'False'), CONVERT(BIT, 'False'))
GO

-- soft delete inbound record
ALTER PROCEDURE zones_entry.sp_delete_inbound (@id INT,
@deleted BIT)
AS
BEGIN
...
SELECT
 ...
 ,header.job_status AS job_status
 ...
 ,line.line_no AS line_invoice_line_number
