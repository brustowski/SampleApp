...
ADD invoice_line_number AS ([zones_entry].[fn_invoice_line_number]([parent_record_id], [id]))
GO
...
DELETE FROM zones_entry.form_configuration
WHERE (column_name = 'filer_code' AND section_id = @decl_id)
OR (column_name = 'team_no' AND section_id = @misc_id)
OR (column_name = 'ftz_date' AND section_id = @lines_id)
OR (column_name = 'line_no' AND section_id = @lines_id)
GO
