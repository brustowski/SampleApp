-- gets  invoice total
Create FUNCTION [zones_entry].[fn_calculate_invoice_total] (@invoiceHeaderId INT,@filingHeaderId INT)
RETURNS NUMERIC(28, 15) 
AS
BEGIN
  DECLARE @result NUMERIC(28, 15) = NULL;

  SELECT
    @result = SUM(line.line_price)
  FROM zones_entry.invoice_line line
  WHERE line.parent_record_id = @invoiceHeaderId and line.filing_header_id=@filingHeaderId
...
BEGIN
  ...
  EXEC zones_entry.sp_add_invoice_line @filingHeaderId
	...
	UPDATE zones_entry.invoice_header set invoice_total=[zones_entry].[fn_calculate_invoice_total](@recordId, @filingHeaderId) where filing_header_id=@filingHeaderId and id=@recordId
  FETCH NEXT FROM cur INTO @recordId
