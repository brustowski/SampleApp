...
AS SELECT
 ...
 ,inbnd.prov_prog_tariff
 ,inbnd.invoice_number
 ,inbnd.fta_recon_flagged_not_filed
 ,inbnd.value_recon_flagged_not_filed
...
DECLARE @ReconFlaggedFiledStatement VARCHAR(MAX);
--- Value Recon Flagged and not filed
SET @ReconFlaggedFiledStatement = IIF(@ReconFlaggedFiled & 1 != 0, COALESCE(@ReconFlaggedFiledStatement + 'OR ', '') + '(GenAddOn_US_OtherReconIndicator.XA_Data = ''VL'' AND ReconJobNumbersVL.Value IS NULL)' + @newLine, @ReconFlaggedFiledStatement);
--- Value Recon Flagged and filed
SET @ReconFlaggedFiledStatement = IIF(@ReconFlaggedFiled & 2 != 0, COALESCE(@ReconFlaggedFiledStatement + 'OR ', '') + '(GenAddOn_US_OtherReconIndicator.XA_Data = ''VL'' AND ReconJobNumbersVL.Value IS NOT NULL)' + @newLine, @ReconFlaggedFiledStatement);
--- FTA Recon Flagged and not filed
SET @ReconFlaggedFiledStatement = IIF(@ReconFlaggedFiled & 4 != 0, COALESCE(@ReconFlaggedFiledStatement + 'OR ', '') + '(GenAddOn_US_NAFTAReconIndicator.XA_Data = ''Y'' AND ReconJobNumbersNF.Value IS NULL AND ISNULL(C7.XV_Data, '''') <> ''EXPIRE'')' + @newLine, @ReconFlaggedFiledStatement);
--- FTA Recon Flagged and filed
SET @ReconFlaggedFiledStatement = IIF(@ReconFlaggedFiled & 8 != 0, COALESCE(@ReconFlaggedFiledStatement + 'OR ', '') + '(GenAddOn_US_NAFTAReconIndicator.XA_Data = ''Y'' AND ReconJobNumbersNF.Value IS NOT NULL)' + @newLine, @ReconFlaggedFiledStatement);
--- Not Flagged
SET @ReconFlaggedFiledStatement = IIF(@ReconFlaggedFiled & 16 != 0, COALESCE(@ReconFlaggedFiledStatement + 'OR ', '') + '((GenAddOn_US_OtherReconIndicator.XA_Data IS NULL OR GenAddOn_US_OtherReconIndicator.XA_Data <> ''VL'')' + @newLine 
    + ' AND (GenAddOn_US_NAFTAReconIndicator.XA_Data IS NULL OR GenAddOn_US_NAFTAReconIndicator.XA_Data <> ''Y''))' + @newLine, @ReconFlaggedFiledStatement);
...
SET @myStatement = 'SELECT DISTINCT
 ...
 ,LineSupTariffAddInfo.Value AS prov_prog_tariff
 ,JZ_InvoiceNumber AS invoice_number
 ,IIF(GenAddOn_US_OtherReconIndicator.XA_Data = ''VL'', IIF(ReconJobNumbersVL.Value IS NULL, ''Y'', ''N''), NULL) AS value_recon_flagged_not_filed
 ,IIF(GenAddOn_US_NAFTAReconIndicator.XA_Data = ''Y'', IIF(ReconJobNumbersNF.Value IS NULL AND ISNULL(C7.XV_Data, '''') <> ''EXPIRE'', ''Y'', ''N''), NULL) AS fta_recon_flagged_not_filed 
...
CROSS APPLY dbo.csfn_GetAddInfoValueFromCodeInlineToReturnEmptyIfNull(JI_AddInfo, ''SupTariff'') AS LineSupTariffAddInfo
...
+ COALESCE('AND (' + @newLine + @ReconFlaggedFiledStatement + ')', '')
;
...
DECLARE @tbl TABLE (
 ...
 ,value_recon_flagged_not_filed VARCHAR(1) NULL
 ,fta_recon_flagged_not_filed VARCHAR(1) NULL
)
...
INSERT INTO @tbl (
   ...
   ,prov_prog_tariff
   ,invoice_number
   ,value_recon_flagged_not_filed
   ,fta_recon_flagged_not_filed)
  ...
  WHEN MATCHED
    THEN UPDATE
      SET inbnd.job_number = src.job_number
         ...
         ,inbnd.prov_prog_tariff = src.prov_prog_tariff
         ,inbnd.invoice_number = src.invoice_number
         ,inbnd.value_recon_flagged_not_filed = src.value_recon_flagged_not_filed
         ,inbnd.fta_recon_flagged_not_filed = src.fta_recon_flagged_not_filed
  WHEN NOT MATCHED
    THEN INSERT (
        job_number
       ,importer
      ...
      VALUES (
       ...
       ,src.prov_prog_tariff
       ,src.invoice_number
       ,src.value_recon_flagged_not_filed
       ,src.fta_recon_flagged_not_filed)
  OUTPUT INSERTED.id
        ,$ACTION
        ,GETDATE() AS modified)
...
UPDATE recon.inbound SET
    fta_recon_flagged_not_filed = IIF(nafta_recon = 'Y', IIF(recon_job_numbers_nf IS NULL AND fta_recon_filing <>'EXPIRE' , 'Y', 'N') ,NULL)
    , value_recon_flagged_not_filed = IIF(recon_issue = 'VL', IIF(recon_job_numbers_vl IS NULL, 'Y', 'N') ,NULL);
GO