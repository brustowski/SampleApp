...
AS SELECT
 ...
 ,CAST(IIF((ace_report.reconciliation_indicator = 'Y' AND (inbnd.recon_issue IS NULL OR inbnd.recon_issue = '')
  OR (ace_report.reconciliation_indicator = 'N' AND inbnd.recon_issue <> '')), 1, 0) AS BIT) AS mismatch_recon_value_flag
 ,CAST(IIF(ace_report.nafta_reconciliation_indicator = 'Y' AND (inbnd.nafta_recon IS NULL OR inbnd.nafta_recon <> 'Y'), 1, 0) AS BIT) AS mismatch_recon_fta_flag
 ,CAST(IIF(ace_report.line_goods_value_amount IS NOT NULL AND NOT ace_report.line_goods_value_amount = inbnd.line_entered_value, 1, 0) AS BIT) AS mismatch_entry_value
 ,CAST(IIF(ace_report.line_duty_amount IS NOT NULL AND NOT ace_report.line_duty_amount = inbnd.duty, 1, 0) AS BIT) AS mismatch_duty
 ,CAST(IIF(ace_report.line_mpf_amount IS NOT NULL AND NOT ace_report.line_mpf_amount = inbnd.mpf, 1, 0) AS BIT) AS mismatch_mpf
 ,CAST(IIF(ace_report.line_tariff_quantity IS NOT NULL AND NOT ace_report.line_tariff_quantity = inbnd.customs_qty1, 1, 0) AS BIT) AS mismatch_quantity
 ,CAST(IIF(ace_report.hts_number_full IS NOT NULL AND NOT ace_report.hts_number_full = inbnd.tariff, 1, 0) AS BIT) AS mismatch_hts
 ,CAST(IIF(ace_report.total_paid_mpf_amount IS NULL
  ...
  , 0, 1) AS BIT) AS mismatch_payable_mpf
