...
{
    /// <summary>
    /// Class describing the configuration for the recon value report records grid
    /// </summary>
    public class ReconValueRecordsGridConfig : GridConfiguration<ValueReconViewModel>
    {
        /// <summary>
        /// Gets the name of the grid
        /// </summary>
        public override string GridName => GridNames.ValueRecords;
        ...
        protected override void ConfigureColumns()
        {
            ...
            AddColumn(x => x.FinalUnitPrice).DisplayName("Final Unit Price").MinWidth(110);
            AddColumn(x => x.FinalTotalValue).DisplayName("Final Total Value").MinWidth(110);
        ...
        protected override void ConfigureFilters()
        {
            ...
            SelectFilterFor(x => x.Tariff).DataSourceFrom<TariffDataProvider>().Title("Tariff").SetOperand(FilterOperands.Equal);
            ...
            SelectFilterFor(x => x.PsaReason520d).Title("PSA Reason 520d").DataSourceFrom<PsaReasonFilterDataProvider>().SetOperand(FilterOperands.Custom).NotSearch().IsMultiSelect();
            SelectFilterFor(x => x.Status).Title("Status").DataSourceFrom<StatusFilterDataProvider<ValueReconStatus, int>>().NotSearch().IsMultiSelect();
            DateRangeFilterFor(x => x.CreatedDate).Title("Upload Date");
            SelectFilterFor(x => x.CreatedUser).Title("Upload User").DataSourceFrom<CreatedUserDataProvider<ValueRecon>>().NotSearch();
        }
    }
}
