...
import { styleMappHighlightingType, typeMappHighlightingIcon, valueReconStatusTypes } from '@app/utils';
import { from } from 'rxjs';

@Component({
  selector: 'lxft-value-recon-list',
  templateUrl: './value-recon-list.component.html'
})
export class ValueReconListComponent extends GridPageCtrl implements OnInit, OnDestroy {
  ...
  protected loadInitialParameters(): void {
    ...
    if (pageName) {
      if (this.setPaginationOptions(pageName)) {
        ...
        this.uploadUrl = `imports/upload/grids/${ReconGridNames.ValueRecords}`;
      }
  ...
  protected setStatusColumns() {
    ...
    statusCol.name = 'Value Status';
    this.columns.push(statusCol);
  }

  displayOriginal(row: any, column: Column): boolean {
    return (row.cargoWiseData && (row[column.prop] || '') !== (row.cargoWiseData[column.prop] || ''));
  }
  ...
  getStatusType(status: string | number): string {
    return `${valueReconStatusTypes[status]} no-before`;
  }
  ...
  process(id: number | string): void {
    ...
    if (selecteId.length) {
      this.reconApiService.process(selecteId).subscribe(result => {
        if (result !== selecteId.length) {
          this.toastr.warning('CW returned no Job data for some records.');
        }
        this.getPage();
      });
    }
