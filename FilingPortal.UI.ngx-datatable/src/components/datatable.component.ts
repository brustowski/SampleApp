...
export class DatatableComponent implements OnInit, DoCheck, AfterViewInit {
  ...
  get allRowsSelected(): boolean {
    ...
    let allRowsSelected = (this.rows && this.selected &&
      (this.rows as any[]).every(x => this.selected.find(y => this.rowIdentity(x) === this.rowIdentity(y))));
  ...
  get fullHeight(): number {
    let result = this.headerHeight + this.footerHeight;
  ...
  */
  ...
  onHeaderSelect(event: any): void {
    ...
    if (this.selectAllRowsOnPage) {
      ...
      // before we splice, chk if we currently have all selected
      const allSelected = (this.rows && this.selected &&
        (this.rows as any[]).every(x => this.selected.find(y => this.rowIdentity(x) === this.rowIdentity(y))));
      // remove all existing either way
      this.selected = this.selected.filter(
        x => (this.rows as any[]).find(
          y => this.rowIdentity(x) === this.rowIdentity(y)) === undefined);
      // do the opposite here
      if (!allSelected) {
