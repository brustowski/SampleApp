...
-- review mapped data
ALTER PROCEDURE us_exp_rail.sp_review_entry (@filingHeaderIds VARCHAR(MAX)
...
BEGIN
  ...
  SET @values = (SELECT
    ...
    FROM us_exp_rail.form_configuration AS defValue
    LEFT JOIN isf.form_configuration AS dependancy
      ON defValue.depends_on_id = dependancy.id
    INNER JOIN us_exp_rail.form_section_configuration AS section
