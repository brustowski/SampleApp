...
-- review mapped data
ALTER PROCEDURE isf.sp_review_entry (@filingHeaderIds VARCHAR(MAX)
...
BEGIN
  ...
  SET @values = (SELECT
    ...
    FROM isf.form_configuration AS defValue
    LEFT JOIN isf.form_configuration AS dependancy
      ON defValue.depends_on_id = dependancy.id
    INNER JOIN isf.form_section_configuration AS section
