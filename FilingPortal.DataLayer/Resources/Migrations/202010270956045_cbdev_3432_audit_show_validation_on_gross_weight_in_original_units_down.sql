...
BEGIN
  ...
  DECLARE @customsQtyInKg DECIMAL(18, 9) = NULL;
  ...
  IF (@customsQtyUnit = 'BBL')
  BEGIN
    DECLARE @from DECIMAL(18, 9) = @customsQtyInKg * 0.95;
    DECLARE @to DECIMAL(18, 9) = @customsQtyInKg * 1.05;
    IF NOT (@grossWeightInKg BETWEEN @from AND @to)
    BEGIN
      INSERT INTO @tbl (fields, message)
        VALUES ('gross_weight, customs_qty', CONCAT('Gross Weight should be between (', @from, ' KG) and (', @to, ' KG)'));
    END
  END
  ELSE
  ...
  BEGIN
    INSERT INTO @tbl (fields, message)
      VALUES ('gross_weight, customs_qty', CONCAT('Gross Weight should be (', @customsQtyInKg, ' KG)'));
  END
