IF EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID(N'fn_pipeline_GetImporterCode') AND type IN ('IF', 'FN', 'TF'))
  DROP FUNCTION dbo.fn_pipeline_GetImporterCode
GO
CREATE FUNCTION [dbo].[fn_pipeline_GetImporterCode] 
(
...
DECLARE @ImporterCode varchar(128);  
  ...
  SET @ImporterCode = (SELECT CASE 
		WHEN importerlookup.importer_code  IS NULL 
			THEN inbound.Importer 
...
BEGIN
  ...
  DECLARE @ImporterCode VarCHAR(128);
	--Get Importercode for the inbound Importer
		SELECT @ImporterCode =  [dbo].[fn_pipeline_GetImporterCode] (@Filing_Headers_id)
   ...
   SELECT
    ...
    LEFT JOIN Pipeline_Rule_PortImporter rulePorts
      ON (
		...
		OR
		(-- we have tofacility-port rule but no port-importer rule associated with record 
        RTRIM(LTRIM(rulePorts.port)) = RTRIM(LTRIM(ruleFacility.port))
		AND 
			((LOWER(RTRIM(LTRIM(rulePorts.Importer))) = LOWER('N/A') AND  @ImporterCode NOT IN (SELECT Importer FROM Pipeline_Rule_PortImporter WHERE port =  ruleFacility.port)
			)
			)	
		 )
		OR
		(--we dont have tofacility-port rule but port-importer rule exist for the record 
		(
		(
		RTRIM(LTRIM(@ImporterCode)) = RTRIM(LTRIM(rulePorts.Importer))  
		)
