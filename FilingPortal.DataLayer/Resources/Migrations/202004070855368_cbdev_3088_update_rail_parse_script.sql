IF NOT EXISTS (SELECT
      *
    FROM sys.indexes
    WHERE name = 'Idx__search_duplicates'
    AND object_id = OBJECT_ID('dbo.imp_rail_inbound'))
BEGIN
  CREATE NONCLUSTERED INDEX Idx__search_duplicates
  ON dbo.imp_rail_inbound (equipment_initial, equipment_number, bill_of_lading, port_of_unlading, reference_number1)
  INCLUDE (id);
END
GO

IF OBJECT_ID('dbo.sp_imp_rail_parse_broker_download', 'P') IS NOT NULL
  DROP PROCEDURE dbo.sp_imp_rail_parse_broker_download
GO

CREATE PROCEDURE dbo.sp_imp_rail_parse_broker_download
AS
BEGIN
