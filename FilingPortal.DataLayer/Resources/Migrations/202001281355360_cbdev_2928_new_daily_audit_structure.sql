...
CREATE PROCEDURE dbo.sp_validate_daily_audit
AS
BEGIN
  ...
  IF (@supplierMid != @manufacturerMid)
    INSERT INTO @tbl (fields, message)
      VALUES ('supplier_mid, manufacturer_mid', 'Supplier MID should be equal to Manufacturer MID');
  ----------------------------------------------------------
