...
DELETE rd
  ...
  INNER JOIN dbo.rail_sections rs
    ON rd.section_id = rs.id
WHERE rd.ColName = 'Invoice_Total'
...
--- pipeline ---
DELETE pd
  FROM dbo.Pipeline_DEFValues pd
  INNER JOIN dbo.pipeline_sections ps
    ON pd.section_id = ps.id
WHERE pd.ColName = 'Invoice_Total'
  AND ps.table_name = 'Pipeline_InvoiceHeaders'
GO
