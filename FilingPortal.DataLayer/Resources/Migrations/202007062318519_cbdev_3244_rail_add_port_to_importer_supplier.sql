...
OUTER APPLY (SELECT TOP (1)
  ...
  WHERE irris.importer_name = inbnd.importer
...
OUTER APPLY (SELECT TOP (1)
  ...
  AND (irrp.importer = inbnd.importer
  ...
  ORDER BY irrp.importer DESC, irrp.supplier DESC, irrp.port DESC) AS Rule_Desc1_Desc2_Tariff
  ...
  BEGIN
    ...
    OUTPUT INSERTED.ID INTO @IDs (ID)
      ...
      OUTER APPLY (SELECT TOP (1)
        ...
        AND (irrp.port = inbnd.port_of_unlading
        OR irrp.port IS NULL)
        ORDER BY irrp.importer DESC, irrp.supplier DESC, irrp.port DESC) AS Rule_Desc1
