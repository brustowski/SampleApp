...
CREATE PROCEDURE dbo.sp_cw_exp_truck_update_entry_status
AS
BEGIN
  SET NOCOUNT ON;
  ...
  AND filing_header.last_modified_date > DATEADD(MINUTE, -90, GETDATE())
  ...
  WHERE jdec.je_declarationreference IN (' + @filing_numbers + ')';
    ...
    WHERE header.filing_number IS NOT NULL;
  END
END
GO