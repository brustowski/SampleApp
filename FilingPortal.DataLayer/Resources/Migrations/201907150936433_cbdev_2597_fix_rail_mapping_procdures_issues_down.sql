--- resore Rail Invoice Lines table ---
...
ALTER TABLE dbo.Rail_InvoiceLines
ADD Gr_Weight NUMERIC(18, 6) NULL
GO

ALTER TABLE dbo.Rail_InvoiceLines
ADD Gr_Weight_Unit VARCHAR(2) NULL
GO
...
GO
CREATE FUNCTION dbo.fn_RailWeightSummary (@filingHeaderId INT)
...
BEGIN
  ...
  SELECT TOP (1)
    ...
    CASE
      ...
      ELSE dbo.weightToTon(@calcwt, ril.Gr_Weight_Unit) * 1000
    END
...
GO
CREATE FUNCTION dbo.fn_Rail_CalculateGrossWtUnit (@filingHeaderId INT)
...
--- restore Rail Container tab table ---
IF EXISTS (SELECT
      1
    FROM INFORMATION_SCHEMA.COLUMNS
    WHERE TABLE_NAME = 'Rail_ContainersTab'
    AND COLUMN_NAME = 'Gross_Weight_Unit'
    AND TABLE_SCHEMA = 'DBO')
BEGIN
...
DELETE dbo.Rail_DEFValues
WHERE ColName = 'Gross_Weight_Unit'
  OR ColName = 'Gross_Weight'
GO

DELETE dbo.Rail_DEFValues_Manual
WHERE column_name = 'Gross_Weight_Unit'
  OR column_name = 'Gross_Weight'
GO
...
SELECT
 ...
 ,ISNULL(l.Gr_Weight, 0) AS Gross_weight
 ,ISNULL(l.Gr_Weight_Unit, 0) AS Gross_weight_unit
