ALTER PROCEDURE dbo.sp_cw_imp_rail_get_daily_audit (@dtFrom DATETIME NULL, @dtTo DATETIME NULL)
AS
BEGIN
  ...
  SET @dtStrFrom = CONVERT(VARCHAR(10), COALESCE(@dtFrom, @modifiedFrom), 1)
  DECLARE @dtStrTo VARCHAR(10)
  SET @dtStrTo = CONVERT(VARCHAR(10), COALESCE(@dtTo, @modifiedFrom), 1)
...
SELECT DISTINCT
  ...
  InvoiceLine.JI_CustomAttrib4 As custom_attrib4,
...
WHERE (JE_TRANSPORTMODE=''RAI'' OR JE_TRANSPORTMODE=''TRK'') AND JE_MessageType=''IMP'' AND CONVERT(DATE, LastModifiedDate.XV_Data) >= ''' + @dtStrFrom + ''' AND CONVERT(DATE, LastModifiedDate.XV_Data) <= ''' + @dtStrTo + ''''
  ...
  DECLARE @tbl TABLE (
   ...
   ,custom_attrib4 VARCHAR(50) NULL
