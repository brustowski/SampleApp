...
BEGIN
   ...
   SELECT
    ...
    LEFT JOIN Pipeline_Rule_PortImporter rulePorts
      ON (RTRIM(LTRIM(inbound.Importer)) = RTRIM(LTRIM(rulePorts.Importer))
        AND RTRIM(LTRIM(ruleFacility.port)) = RTRIM(LTRIM(rulePorts.port)))  OR
		(RTRIM(LTRIM(inbound.Importer)) = RTRIM(LTRIM(rulePorts.Importer)) OR 
		RTRIM(LTRIM(ruleFacility.port)) = RTRIM(LTRIM(rulePorts.port)) AND 
		ISNULL(RTRIM(LTRIM(rulePorts.Importer)),'')='') 
  ...
  INSERT INTO Pipeline_InvoiceHeaders (id, Filing_Headers_FK, Invoice_No, Supplier, Invoice_Total, Origin, Consignee, Transaction_Related, Manufacturer, Seller, Importer, Sold_to_party, Ship_to_party,Supplier_Address)
    SELECT DISTINCT
