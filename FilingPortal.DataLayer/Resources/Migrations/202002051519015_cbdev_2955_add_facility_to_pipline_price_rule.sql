CREATE OR ALTER VIEW dbo.v_imp_pipeline_rule_price 
WITH VIEW_METADATA
AS WITH CTE
AS
(SELECT
   ...
   ,rule_price.pricing
   ,rule_price.freight
   ,rule_price.crude_type_id
   ,rule_price.facility_id
   ,CASE
      WHEN rule_price.crude_type_id = rule_batch.id AND
        rule_price.facility_id = rule_facility.id THEN 4
      WHEN rule_price.crude_type_id = rule_batch.id AND
        rule_price.facility_id IS NULL THEN 3
      WHEN rule_price.crude_type_id IS NULL AND
        rule_price.facility_id = rule_facility.id THEN 2
      WHEN rule_price.crude_type_id IS NULL AND
        rule_price.facility_id IS NULL THEN 1
      ELSE 0
    END AS weight
  FROM imp_pipeline_rule_price rule_price
  JOIN Clients c
    ON rule_price.importer_id = c.id
  JOIN imp_pipeline_inbound inbnd
    ON c.ClientCode = inbnd.importer
  LEFT JOIN dbo.imp_pipeline_rule_batch_code rule_batch
    ON dbo.fn_imp_pipeline_batch_code(inbnd.batch) = rule_batch.batch_code
  LEFT JOIN imp_pipeline_rule_facility rule_facility
    ON inbnd.facility = rule_facility.facility
  WHERE rule_price.importer_id = c.id)
SELECT
  id
 ,pricing
 ,freight
...
CREATE OR ALTER VIEW dbo.v_imp_pipeline_inbound_grid
AS
SELECT
  inbnd.id
 ,inbnd.importer
 ...
 ,CASE
    WHEN EXISTS (SELECT
          1
        FROM dbo.v_imp_pipeline_rule_price rule_price
        WHERE inbnd.id = rule_price.id) THEN 1
    ELSE 0
  ...
  BEGIN
    ...
    OUTPUT INSERTED.ID INTO @IDs (ID)
      SELECT DISTINCT
       ...
       ,inbnd.quantity * rule_price.pricing
      ...
      LEFT JOIN dbo.v_imp_pipeline_rule_price rule_price
        ON inbnd.id = rule_price.id
      LEFT JOIN dbo.imp_pipeline_rule_batch_code rule_batch
        ON dbo.fn_imp_pipeline_batch_code(inbnd.Batch) = rule_batch.batch_code
  ...
  BEGIN
    INSERT INTO imp_pipeline_invoice_line (
      ...
      SELECT
       ...
       ,rule_importer.origin
       ,inbnd.quantity * rule_price.pricing
       ,inbnd.quantity * rule_price.freight
       ,rule_importer.manufacturer
       ,rule_importer.consignee
       ,rule_importer.consignee
       ,rule_importer.origin
