...
IF NOT EXISTS (SELECT
    ...
    AND type IN ('IF', 'FN', 'TF'))
  EXEC sp_executesql N'CREATE FUNCTION dbo.fn_pipeline_weight (
  @bbl NUMERIC(18,6) = 0,
  @api NUMERIC(18,6) = 0)
RETURNS NUMERIC(18,6)
AS BEGIN
   	DECLARE @result NUMERIC(18,6) = 0
    SELECT @result = (@bbl*42*(((141.5/(131.5+@api))*8.338426855)-0.0101578))*0.453592/1000
    RETURN @result
   END
'
GO
...
CREATE PROCEDURE dbo.pipeline_filing (@Filing_Headers_id INT,
...
BEGIN
  ...
  , Manifest_QTY)
    SELECT
     ...
     ,inbound.EntryNumber -- Bill_Num - varchar(128)
...
CREATE PROCEDURE dbo.pipeline_filing_param (@Filing_Headers_id INT = 117)
AS
BEGIN
