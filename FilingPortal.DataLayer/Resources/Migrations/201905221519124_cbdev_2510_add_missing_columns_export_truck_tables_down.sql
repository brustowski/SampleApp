ALTER TABLE [dbo].[truck_export_declarations]
 DROP column [check_local_client] 
 Go

  ALTER table [dbo].[truck_export_invoice_headers]
   DROP column [ultimate_consignee] 
   GO
...
BEGIN
  ...
  BEGIN
    ...
    INSERT INTO dbo.truck_export_invoice_headers (filing_header_id, usppi_address, usppi_contact, usppi_phone, ultimate_consignee_type,usppi,invoice_total_amount)
      SELECT
        @parentId
       ,address
       ,contact
  ...
  BEGIN
    ...
    (SELECT
      ...
      LEFT JOIN Truck_Export_Rule_Exporter_Consignee terec
        ...
        AND te.exporter = terec.exporter
      WHERE tefd.filing_header_id = @filingHeaderId)
    INSERT INTO dbo.truck_export_declarations (filing_header_id, destination, country_of_dest, tran_related, routed_tran, eccn, export, importer, tariff_type, sold_en_route, master_bill,owner_ref,port_of_loading,transport_ref,main_supplier,dep,arr_date,exp_date)
      SELECT
        @parentId
       ,destination
       ,country
