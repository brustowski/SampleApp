...
AS SELECT
 ...
 ,inbnd.deleted
 ,inbnd.created_date
 ,COALESCE(update_record.created_date, filing_header.last_modified_date) AS modified_date
 ,COALESCE(update_record.created_user, filing_header.last_modified_user) AS modified_user
 ,filing_header.id AS filing_header_id
 ,filing_header.filing_number
 ,filing_header.job_link
 ,COALESCE(update_record.auto_refile, filing_header.is_auto_filed) AS is_auto_filed
 ,filing_header.is_updated
 ,IIF(update_record.created_date IS NOT NULL AND CAST(update_rule_consignee.found & update_rule_exporter_consignee.found AS BIT) <> 1, 2, COALESCE(filing_header.update_status, 0)) AS update_status
...
OUTER APPLY (SELECT
    IIF(EXISTS (SELECT
        1
      FROM exp_truck_rule_consignee AS rule_consignee
      WHERE rule_consignee.consignee_code = COALESCE(update_record.importer, inbnd.importer))
    , 1, 0) AS found) AS update_rule_consignee
OUTER APPLY (SELECT
    IIF(EXISTS (SELECT
        1
      FROM exp_truck_rule_exporter_consignee AS rule_exporter_consignee
      WHERE rule_exporter_consignee.consignee_code = COALESCE(update_record.importer, inbnd.importer)
      AND rule_exporter_consignee.exporter = COALESCE(update_record.exporter, inbnd.exporter))
    , 1, 0) AS found) AS update_rule_exporter_consignee

WHERE inbnd.deleted = 0
GO

DROP PROCEDURE dbo.sp_exp_truck_validate
GO