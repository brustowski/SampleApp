...
BEGIN
  ...
  Importer_of_record
   ...
   SELECT
     ...
     ,ruleImporters.Importer AS Importer
    ...
    LEFT JOIN Pipeline_Rule_Importer ruleImporters
       ON (RTRIM(LTRIM(inbound.Importer)) = RTRIM(LTRIM(ruleImporters.Importer))  OR RTRIM(LTRIM(importerlookup.importer_code)) = RTRIM(LTRIM(ruleImporters.Importer))
	     )
    LEFT JOIN Pipeline_Rule_PortImporter rulePorts
      ON (
		--we have both tofacility-port and port-importer rule associated with record
		(
		(RTRIM(LTRIM(inbound.Importer)) = RTRIM(LTRIM(rulePorts.Importer)) OR RTRIM(LTRIM(importerlookup.importer_code)) = RTRIM(LTRIM(rulePorts.Importer)))
