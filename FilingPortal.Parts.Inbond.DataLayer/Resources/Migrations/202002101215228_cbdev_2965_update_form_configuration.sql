...
UPDATE cfg
SET cfg.marks_remarks = 1, display_on_ui = 0
...
WHERE cfg.column_name IN ('template_type', 'description', 'marks_and_numbers');
...
AND NOT EXISTS (SELECT
    1
  FROM inbond.form_configuration AS cfg
  WHERE cfg.marks_remarks = 1
  AND cfg.column_name = i.COLUMN_NAME
  AND cfg.section_id = s.id)
GO