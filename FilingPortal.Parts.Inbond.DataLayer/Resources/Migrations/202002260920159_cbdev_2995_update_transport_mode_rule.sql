MERGE
INTO inbond.rule_transport_mode AS target
USING (SELECT
    importer_id
   ,firms_code_id
   ,consignee_id
   ,carrier
  FROM inbond.rule_carrier) AS source
ON (source.carrier = target.carrier)
WHEN MATCHED
  THEN UPDATE
    SET importer_id = source.importer_id
       ,firms_code_id = source.firms_code_id
       ,consignee_id = source.consignee_id
WHEN NOT MATCHED BY SOURCE
  THEN DELETE;
GO

ALTER TABLE inbond.rule_transport_mode
ALTER COLUMN importer_id UNIQUEIDENTIFIER NOT NULL;
ALTER TABLE inbond.rule_transport_mode
ALTER COLUMN firms_code_id INT NOT NULL;
ALTER TABLE inbond.rule_transport_mode
ALTER COLUMN consignee_id UNIQUEIDENTIFIER NOT NULL;
GO
