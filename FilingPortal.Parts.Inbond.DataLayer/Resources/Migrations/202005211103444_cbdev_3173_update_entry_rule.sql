UPDATE rule_entry
SET transport_mode = rule_transport_mode.transport_mode
FROM inbond.rule_entry AS rule_entry
LEFT JOIN inbond.rule_transport_mode AS rule_transport_mode
  ON rule_transport_mode.importer_id = rule_entry.importer_id
  AND rule_transport_mode.consignee_id = rule_entry.consignee_id
  AND rule_transport_mode.firms_code_id = rule_entry.firms_code_id
  AND rule_transport_mode.carrier = rule_entry.carrier
WHERE rule_entry.transport_mode IS NULL
OR rule_entry.transport_mode = '';
GO

ALTER TABLE inbond.main_detail
ADD importer_address VARCHAR(128) NULL;
GO
...
VALUES (
 ...
 ,(SELECT
    ...
    FROM inbond.form_configuration AS config
    WHERE config.section_id = 2
    AND config.column_name = 'importer'
    ORDER BY config.id
    OFFSET 0 ROW FETCH NEXT 1 ROW ONLY)
 ,'system.ClientAddressCode');
GO

UPDATE inbond.form_configuration
SET handbook_name = 'system.ClientCode'
WHERE section_id = 2
AND column_name = 'importer';
GO